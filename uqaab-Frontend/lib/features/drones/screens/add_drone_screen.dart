...
class _AddDroneScreenState extends State<AddDroneScreen> {
  ...
  Future<void> _handleSave() async {
    ...
    if (pid == null) return;
    final success = await context.read<DroneProvider>().createDrone(propertyId: pid, name: _nameController.text.trim(), connectionString: _connectionController.text.trim(), row: _selectedRow, col: _selectedCol);
    if (!mounted) return;
    if (success) { Helpers.showSuccessSnackBar(context, 'Drone added!'); Navigator.of(context).pop(); }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: const CustomAppBar(title: 'Add New Drone'),
      body: SingleChildScrollView(
        ...
        child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 16),
          const Text('Home Location (Grid Cell) *', style: TextStyle(color: AppColors.textSecondary, fontSize: 14, fontWeight: FontWeight.w500)),
          ...
          Consumer<DroneProvider>(builder: (context, p, _) => CustomButton(text: 'SAVE DRONE', isLoading: p.isLoading, onPressed: _handleSave)),
        ])),
      ),
    );
  }
